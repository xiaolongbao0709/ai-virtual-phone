...
import { RoleThemeCustomizer, DEFAULT_ROLE_THEME_PROFILE, type RoleThemeProfile } from "./role-theme-customizer";
...
export function CheckPhoneApp({ onClose }: CheckPhoneAppProps) {
  ...
  const [showRoleThemeCustomizer, setShowRoleThemeCustomizer] = useState(false);
  const [roleThemeMap, setRoleThemeMap] = useState<Record<string, RoleThemeProfile>>({});
  ...
  const activeRoleTheme = activeCharId
    ? roleThemeMap[activeCharId] ?? DEFAULT_ROLE_THEME_PROFILE
    ...
    return (
      <div className="cp-fullscreen-simulator">
        <div className={`cp-fullscreen-inner ${!manifest ? "is-empty" : ""}`}>
          ...
          {!isEmbeddedAppOpen && (
            <div className="cp-floating-controls">
              <div className="cp-floating-settings" ref={settingsPanelRef}>
                ...
                <button
                  className="cp-float-settings"
                  onClick={() => setShowRoleThemeCustomizer(true)}
                  aria-label="Role theme customizer"
                >
          ...
          {showRoleThemeCustomizer && activeCharacter ? (
            <RoleThemeCustomizer
              activeRoleName={activeCharacter.name}
              profile={activeRoleTheme}
              onChange={(next) => {
                if (!activeCharId) return;
                setRoleThemeMap((prev) => ({
                  ...prev,
                  [activeCharId]: next,
                }));
              }}
              onApply={() => setShowRoleThemeCustomizer(false)}
              onClose={() => setShowRoleThemeCustomizer(false)}
            />
          ) : null}
